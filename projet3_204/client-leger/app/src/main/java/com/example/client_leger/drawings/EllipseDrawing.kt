...
class EllipseDrawing(start: Point, endPoint: Point, paint: Paint, style: Style, originHeight: Int, originWidth: Int) : ADrawing(originHeight, originWidth) {
    ...
    override fun draw(canvas: Canvas?) {
        ...
        if(canvas != null){
            if(path.isEmpty){
                val sqrDiffX = (endPoint.x-startPoint.x)*(endPoint.x-startPoint.x)
                val sqrDiffY = (endPoint.y-startPoint.y)*(endPoint.y-startPoint.y)

                val raduis = sqrt(sqrDiffX + sqrDiffY) /2F
                ...
                if(sqrDiffX == sqrDiffY){
                    path.addCircle(((endPoint.x+startPoint.x)*ratios.second/2F),
                        ((endPoint.y+startPoint.y)*ratios.first/2F),raduis,Path.Direction.CW)
                }else{
                    val ovalRect = RectF(startPoint.x*ratios.second,
